    ...
    >
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        ...
        <div className="mt-12 grid gap-5 sm:mt-16 lg:mt-20 lg:grid-cols-2 lg:items-stretch lg:gap-8">
          <SectionReveal delay={0.04} className="h-full min-h-0">
          ...
          <SectionReveal delay={0.08} className="h-full min-h-0">
            <article className="rs-surface-enterprise flex h-full min-h-[min(22rem,70vh)] flex-col rounded-2xl p-7 sm:rounded-3xl sm:p-9">
              ...
              <ul className="mt-4 space-y-2.5 border-t border-white/[0.05] pt-4 sm:mt-5" role="list">
                {site.split.enterprise.pillars.map((row) => (
                  <li key={row.t} className="flex gap-2 text-sm sm:text-base">
                    <span className="w-[5.5rem] shrink-0 font-medium text-rs-text sm:w-32">{row.t}</span>
